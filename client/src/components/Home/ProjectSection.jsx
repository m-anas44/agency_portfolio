        ...
        <div className='container py-5 mx-auto'>
          <div className='grid grid-cols-1 gap-8 mt-8 lg:mt-8 lg:gap-12 lg:grid-cols-2 '>
            {portfolioItems.map((item) => (
              <div
                key={item.id}
                className='relative flex flex-col items-start p-4 overflow-hidden transition-transform duration-300 transform rounded-lg shadow-xl group hover:scale-105'
              >
                {/* Image */}
                <img src={item.image} alt={item.title} className='object-cover w-full h-64 rounded-t-lg' />

                {/* Button (hidden by default, visible on hover) */}
                <div className='absolute inset-0 flex items-center justify-center p-4 transition-opacity duration-300 opacity-0 bg-gray-400/30 group-hover:opacity-100'>
                  <button className='px-6 py-2 text-lg font-medium text-white rounded-full bg-primary hover:bg-secondary'>
                    Watch Live
                  </button>
                </div>

                <div className='w-full px-8 py-2 overflow-hidden rounded-b-lg bg-primary'>
