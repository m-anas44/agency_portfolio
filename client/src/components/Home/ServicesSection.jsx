    ...
    <section className='py-16 bg-white text-primary'>
      <div className='container px-6 mx-auto md:px-12 lg:px-20'>
        ...
        <div className='flex flex-col items-center justify-between mb-10 md:flex-row'>
          ...
          <div>
            <Button className='mt-4 text-lg rounded-full drop-shadow-lg md:text-xl md:mt-0' onClick={() => {}}>
              All Services
            </Button>
          </div>
        </div>
        ...
        <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
          {services.map((service, index) => (
            <div
              key={index}
              className='relative flex flex-col justify-between p-6 overflow-hidden border rounded-lg shadow-lg border-secondary/50 group text-secondary group-hover:border-primary ' // Add `group` to each card
            >
              {/* Sliding Overlay */}
