...
const RecentBlog = () => {
  ...
  const recentPosts = allPosts.sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 4);
      ...
      <ul className='space-y-4'>
        {recentPosts.map((post, index) => (
          ...
          >
            ...
            >
              {/* Post thumbnail */}
              <img src='https://via.placeholder.com/80' alt='Post Thumbnail' className='w-16 h-16 rounded-md' />

              {/* Post title and date */}
              <div>
                <h3 className='text-sm font-semibold'>{post.title}</h3>
                <p className='mt-1 text-xs text-gray-500 group-hover:text-secondary'>📅 {post.date}</p>
              </div>
            </Link>
          </li>
        ))}
